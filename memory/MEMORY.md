...
- 2026-09-11: Mail HTML links in the current page lacked a click interceptor, so anchors without `target` navigated the Electron main window. Added renderer `openExternal` handling in current/legacy mail views and a `will-navigate` main-process defense. App release still required for installed clients.
