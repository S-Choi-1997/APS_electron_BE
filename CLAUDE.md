...
npm run release              # 현재는 electron:build와 동일한 로컬 빌드 별칭
...
- **개발 환경**: 필요에 따라 로컬 백엔드 또는 Cloudflare 백엔드 도메인을 가리킴
- **프로덕션**: Cloudflare Tunnel로 연결된 백엔드 도메인을 가리킴 (`app/.env`의 `VITE_API_URL`)
...
| 변수 | 개발 | 프로덕션 |
|------|------|---------|
| `VITE_API_URL` | `http://localhost:3001` 또는 Cloudflare 백엔드 URL | `https://backend.apsconsulting.kr` |
| `VITE_WS_URL` | 선택 사항. 없으면 API URL에서 파생 | 선택 사항. 없으면 `wss://`로 파생 |
| `VITE_BACKEND_ENVIRONMENT` | `development` | `production` |
...
- **네트워크**: 프로덕션 앱은 Cloudflare 백엔드 도메인으로 직접 접속. 백엔드는 GCP Firestore/Storage 접근을 위한 인터넷 필요.
- **자동 업데이트**: 현재 운영 배포는 서브컴 로컬 빌드 기준. `electron-updater` 코드는 기본 비활성화되어 있으며 `APS_ENABLE_AUTO_UPDATE=true`를 명시한 경우에만 동작함.

## 릴리스 프로세스

자세한 내용은 `docs/release.md` 참고. 요약:
1. `package.json`의 버전 업데이트
2. `app/.env`의 `VITE_API_URL`을 Cloudflare 백엔드 도메인으로 설정
3. 서브컴에서 `npm run electron:build` 실행
4. 백엔드는 NAS/백엔드 서버에서 `BACKEND_IMAGE_TAG`가 가리키는 Docker Hub 이미지를 pull해서 실행
