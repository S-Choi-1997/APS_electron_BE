...
`npm run electron:build` first runs `npm run prepare:app-config`. That command generates `app/electron/app-config.default.json` from `VITE_API_URL`, `VITE_WS_URL`, and `VITE_BACKEND_ENVIRONMENT`. The generated file is ignored by git, but it is included in the packaged app through the Electron Builder `electron/**/*` files rule. This is the packaged runtime default used before any per-user AppConfig override exists.

`VITE_RELAY_ENVIRONMENT` is still accepted as a compatibility fallback, but new release configuration should use `VITE_BACKEND_ENVIRONMENT`.
...
# edit .env with real secrets and API credentials before first deployment
docker-compose pull
docker-compose up -d
...
The health response should show `relayEnabled: false` and `directWebSocket.enabled: true`.
